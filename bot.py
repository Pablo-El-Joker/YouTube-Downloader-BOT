...
class Chat:
    def __init__(self, msg):
        ...
        self.user_input = self.user_input.replace('@YouTube11_11Bot', '')
    ...
    def process_request(self, user_input):
        ...
        if int(min_duration) < 30 and split_count < 3:
            file_name = song.get_title(self, result) +' - @YouTube11_11Bot '+str(randint(0,999999))+'.mp3'
            file_name = file_name.replace('"', '')

            self.send_message(f"🎵 {song.get_title(self, result)}\n🔗 {song.get_link(self, result)}")
            downloading_message = self.send_message('⬇️ التنزيل ... \n_(هذا قد يستغرق بعض الوقت.)_')
